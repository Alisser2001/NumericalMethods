...
def MidPoint(f, fn, a, b, n):
    h = (b - a) / n
    s = 0
    for i in range(0, n):
        xi = a + (i + 0.5) * h
        s += f(xi)
        print("{:^30} {:^30} {:^30} {:^30}".format(str(i + 1), str(xi), str(f(xi)), str(s*h)))
    return s*h
...
def Solution():
    ...
    print("")
    print("{:^30} {:^30} {:^30} {:^30}".format("i", "Xi", "f(Xi)", "Area Total"))
    print("")
    print("\nEl valor aprox. De la integral definida es: ", MidPoint(f, fn, xl, xu, n))
